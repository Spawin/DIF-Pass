...
void main() {
  ...
  testWidgets(
    'shows the QR frame, corner badge, and generated-by mention',
    (tester) async {
      final fakeEvents = FakeEventRepository(
        events: [
          Event(
      ...
      expect(frameDecoration.border, isNotNull);

      final badge = tester.widgetList<Container>(find.byType(Container)).where(
        (c) => (c.decoration as BoxDecoration?)?.image != null,
      );
      expect(badge, isNotEmpty);

      expect(find.textContaining('Generated by'), findsOneWidget);
      expect(find.textContaining('DIF Pass'), findsOneWidget);
    },
  );
